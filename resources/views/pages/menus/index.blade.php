...
@section('header-actions')
    <a class="hubers-btn" href="{{route('menus.create')}}">{{__('general.create_btn')}}</a>
@endsection
@section('content')
    <div class="vms_panel">
        <form class="filter-items" action="/menus" method="GET" >
            <div class="filter-options">
                ...
                <div class="export-options" onclick="exportOptions.export('/menus/export','menus-export.xlsx')">
            ...
            <div class="hubers-filter-options @if ($is_on_search) active @endif">
                ...
                <div class="hubers-filter-list-actions">
                    ...
                    <a href="/menus" class="hubers-btn inverse">{{__('general.filter_reset_btn')}}</a>
                </div>
            </div>
        </form>
        @if(count($menus) > 0)
            <div class="table-responsive p-0">
                <table class="bug-table">
                    ...
                    <tr>
                        ...
                        <th>{{__('menu.table.name')}}</th>
                        <th>{{__('menu.table.price')}}</th>
                        <th>{{__('menu.table.description')}}</th>
                        <th></th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($menus as $menu)
                        <tr>
                            <td>
                                <input class="table-checkbox bug-checkbox-input" type="checkbox" value="{{$menu->id}}">
                            </td>
                            <td>
                                {{$menu->name}}
                            </td>
        ...
        @else
            <div class="hubers-empty-tab">
                @if ($is_on_search)
                    <h5 class="text-center">{{__('menu.table.not_found_with_search')}}</h5>
                    @else
                    <h5 class="text-center">{{__('menu.table.not_found_without_search')}}</h5>
                @endif
            </div>
        @endif
    </div>
